...
async function regenerateAllEmbeddings() {
  ...
  try {
    ...
    console.log(`   Correct embeddings (3072d): ${entriesWithCorrectEmbeddings}`);
    ...
    for (let i = 0; i < allEntries.length; i += batchSize) {
      ...
      for (const entry of batch) {
        ...
        try {
          console.log(`🔄 Processing entry ${processed + 1}/${entriesToUpdate}:`);
          console.log(`   Content: "${entry.content.substring(0, 80)}..."`);
          
          // Generate new embedding using Gemini
          const newEmbedding = await embeddingService.generateEmbedding(entry.content);
          ...
          processed++;
          console.log(`✅ Updated embedding (${newEmbedding.length} dimensions)`);
    ...
    console.log(`📊 All embeddings now use 3072 dimensions (Gemini embedding-001)`);
